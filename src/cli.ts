...
yargsInstance
    .command(
        'create',
        chalk.dim('Create form component following SHACL shape'),
        (args) => {
            return args
        ...
        async (argv: ArgumentsCamelCase<createOpt>) => {
    ...
    .command(
        'list-shapes',
        chalk.dim('List of available shapes (SHACL .ttl files)'),
        () => {},
    ...
    .command(
        'set-custom-css',
        chalk.dim('Set up custom CSS classes to be used in the form component'),
        () => {},
        async (argv: Arguments) => {
            await createClasses()
