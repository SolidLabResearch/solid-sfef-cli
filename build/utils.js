 ...
 */
export function getBasicInputType(element) {
    const type = BASIC_INPUT_MAP.get('inputType');
    const shapeType = eval(type);
    const componentValues = BASIC_TYPE_MAP.get(shapeType);
 ...
 * @returns {Promise}
 */
function classQuestion(element) {
    const rl = readline.createInterface({
        input: process.stdin,
        output: process.stdout
    });
    return new Promise((resolve, reject) => {
        rl.question(`Custom class for the ${element} element: `, (answer) => {
            rl.close();
            resolve(answer);
        });
    });
...
export async function createClasses() {
    ...
    for (const element of FORM_CUSTOM_ELEMENTS) {
        const elementClass = await classQuestion(element);
 ...
 */
export function getClasses(targetElement, customCSS) {
    let classes = '';
    if (customCSS.length) {
